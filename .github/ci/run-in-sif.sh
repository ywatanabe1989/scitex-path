...
# the shared ci-cpu.sif bakes scitex-dev[all,dev] DEPS, NOT scitex-path's —
...
# so only scitex-path's own thin dep set is fetched per run.
#
...
export TMPDIR="/tmp/ci-scitex_path-${GITHUB_RUN_ID:-0}-${GITHUB_RUN_ATTEMPT:-0}-$V"
rm -rf "$TMPDIR"
...
# scitex-path's reproducibility tests (validate_recipe renders the SAME recipe
...
# Install scitex-path + its [all,dev] extras WITH deps into the writable target.
# Fallback chain mirrors scitex-path's historical bare-uv/pip workflow so a
...
# scitex-path's suite is ~2460 tests; single-process it overran the job's old
...
# scitex-path style-stack are naturally isolated per worker — the safe way to
# parallelise a matplotlib-heavy suite.
#
# Worker count: use ALL cores. Each matrix leg now runs on its own dedicated
# self-hosted node (one runner per node: scitex-path-01/02/03), so there is no
...
# loadscope pins an entire MODULE's tests to ONE worker — and scitex-path's heavy
...
exec nice -n 19 ionice -c 3 \
    ...
    --cov=src/scitex_path --cov-report=xml --cov-report=term \
    -p no:cacheprovider
