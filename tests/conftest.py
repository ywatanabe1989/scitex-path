...
This module also wires subprocess coverage at import time
(parallel + COVERAGE_PROCESS_START + idempotent .pth shim) so
coverage data emitted by child Python interpreters (e.g. the
example smoke tests under tests/integration/) is captured.
`os.environ.setdefault` would be a no-op here because pytest-cov
has already set COVERAGE_FILE to a tmp dir by the time conftest
is loaded — we therefore FORCE-set both vars.
"""
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_path_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
